...
function DestinationsSection({
  ...
  content: HomepageFeaturedDestinationsSection;
  destinations: DestinationRow[];
}) {
  const featured = content.destinationIds
    .map((id) => destinations.find((d) => d.id === id))
    .filter((d): d is DestinationRow => Boolean(d));
              ...
              <div className="flex items-center justify-between">
                <span className="text-white/80 text-xs">
                  ...
                  <strong className="text-white font-semibold">
                    {dest.starting_price_xof.toLocaleString("fr-FR")} FCFA
                  </strong>
                </span>
...
function TestimonialsSection({
  ...
  const ordered = content.testimonialIds
    ...
    .filter((t): t is TestimonialRow => Boolean(t));
              ...
              <div className="flex items-center justify-center gap-3">
                {t.user_avatar && (
                  <div className="relative w-11 h-11 rounded-full overflow-hidden border-2 border-luxe-gold/50">
                    <Image src={t.user_avatar} alt={t.user_name} fill className="object-cover" />
                  </div>
                )}
                <div className="text-left">
                  <p className="text-sm font-semibold">{t.user_name}</p>
                  <p className="text-xs text-white/60">{t.trip_title}</p>
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
...
export function LandingPageClient({ content, testimonials, destinations }: LandingPageClientProps) {
  ...
  const sectionRenderers: Record<string, React.ReactNode> = {
    ...
    destinations: (
      <DestinationsSection key="destinations" content={content.featuredDestinations} destinations={destinations} />
    ),
