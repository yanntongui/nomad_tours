...
import type { TripReportListRow } from "@/lib/server/trips";
import type { TripReportManualFields, TripReportStatus } from "@/lib/admin/types";
...
interface Row {
  report: TripReportListRow;
  title: string;
  avgRating: string;
}

export function RapportsClient({ reports }: { reports: TripReportListRow[] }) {
  ...
  const rows = React.useMemo<Row[]>(() => {
    return reports
      .filter((report): report is TripReportListRow & { trips: NonNullable<TripReportListRow["trips"]> } => report.trips !== null)
      .map((report) => {
        const feedbacks = report.trips.trip_feedbacks;
        const avg = feedbacks.length > 0 ? feedbacks.reduce((sum, f) => sum + f.rating, 0) / feedbacks.length : 0;
        return {
          report,
          title: report.trips.bookings?.reference_label ?? "Voyage",
          avgRating: feedbacks.length > 0 ? avg.toFixed(1) : "—",
        };
      });
  }, [reports]);
  ...
  const filtered = React.useMemo(() => {
    return rows.filter((r) => {
      ...
      const matchesDate = !date || r.report.generated_at.slice(0, 10) === date;
  ...
  const columns = React.useMemo<ColumnDef<Row, any>[]>(
    () => [
      {
        accessorKey: "report.generated_at",
        header: "Généré le",
        cell: ({ row }) => <span className="whitespace-nowrap text-xs text-stone-500 dark:text-stone-400">{formatDate(row.original.report.generated_at)}</span>,
      },
      {
        accessorKey: "title",
        header: "Circuit",
        cell: ({ row }) => (
          <Link
            href={`/admin/voyages/${row.original.report.trip_id}/rapport`}
      ...
      },
      {
        ...
        header: "Rédacteur",
        cell: ({ row }) => {
          const manual = (row.original.report.manual ?? {}) as TripReportManualFields;
          return <span className="text-sm text-stone-600 dark:text-stone-300">{manual.redacteur ?? "—"}</span>;
        },
      },
      {
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          onClick={() =>
            exportRowsToCsv(
              filtered.map((r) => {
                const manual = (r.report.manual ?? {}) as TripReportManualFields;
                return {
                  generatedAt: formatDate(r.report.generated_at),
                  circuit: r.title,
                  statut: STATUS_LABELS[r.report.status],
                  redacteur: manual.redacteur ?? "",
