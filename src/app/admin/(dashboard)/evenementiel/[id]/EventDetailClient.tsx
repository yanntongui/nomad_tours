...
import {
  ...
  setQuoteAmountAction,
  setEventServicesAction,
  addEventNoteAction,
} from "../actions";
import type { EventRequestRow } from "@/lib/server/events";
...
export function EventDetailClient({ event }: { event: EventRequestRow }) {
  const router = useRouter();
  const { user } = useAdminRole();
  const [quoteInput, setQuoteInput] = React.useState(event.quote_amount_xof?.toString() ?? "");
  const [noteText, setNoteText] = React.useState("");
  const [cancelOpen, setCancelOpen] = React.useState(false);

  React.useEffect(() => {
    setQuoteInput(event.quote_amount_xof?.toString() ?? "");
  }, [event.id, event.quote_amount_xof]);

  const timelineItems = event.event_timeline
    .slice()
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-lg font-bold text-stone-800 dark:text-stone-100">{event.type} — {event.clients?.name ?? "—"}</h1>
          <p className="text-sm text-stone-500 dark:text-stone-400">
            {formatDate(event.event_date)} — {event.location ?? "Lieu à définir"} — {event.guest_count} invités — Agent : {event.admin_profiles?.name ?? "—"}
          </p>
        </div>
        ...
        <Card>
          <CardContent className="py-6 overflow-x-auto">
            <StatusWorkflowBar
              ...
              onAdvance={async (key) => {
                await advanceEventStatusAction(event.id, key as EventStatus, user.name);
                router.refresh();
              }}
            />
          </CardContent>
              ...
              <CardContent>
                <TagListInput
                  ...
                  onChange={async (services) => {
                    await setEventServicesAction(event.id, services);
                    router.refresh();
                  }}
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div>
                  ...
                  <p className="font-semibold text-stone-800 dark:text-stone-100">{formatXOF(event.budget_xof)}</p>
                </div>
                ...
                <Button
                  size="sm"
                  className="w-full"
                  disabled={!quoteInput || Number(quoteInput) === (event.quote_amount_xof ?? -1)}
                  onClick={async () => {
                    await setQuoteAmountAction(event.id, Number(quoteInput), user.name);
                    router.refresh();
                  }}
                >
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <Button
                ...
                onClick={async () => {
                  await addEventNoteAction(event.id, noteText.trim(), user.name);
                  setNoteText("");
                  router.refresh();
                }}
              >
      ...
      <ConfirmDialog
        ...
        onConfirm={async (reason) => {
          await advanceEventStatusAction(event.id, "CANCELLED", user.name, reason);
          router.refresh();
        }}
      />
    </div>
