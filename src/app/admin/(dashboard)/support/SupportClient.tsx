...
import type { SupportListRow } from "@/lib/server/support";
import type { Tables } from "@/lib/server/types";

type TicketType = Tables<"support_tickets">["type"];
type TicketChannel = Tables<"support_tickets">["channel"];
...
function emptyForm(clients: ClientRef[]) {
  return {
...
function NewTicketDialog({
  ...
  const [form, setForm] = React.useState(() => emptyForm(clients));
  const [submitting, setSubmitting] = React.useState(false);

  React.useEffect(() => {
    if (open) setForm(emptyForm(clients));
  }, [open, clients]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button disabled={!valid || submitting} onClick={submit}>Créer le ticket</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export function SupportClient({ tickets, clients }: { tickets: SupportListRow[]; clients: ClientRef[] }) {
  const router = useRouter();
  ...
  const filtered = React.useMemo(() => {
    return tickets.filter((t) => {
      ...
      if (search && !clientName.toLowerCase().includes(search.toLowerCase()) && !t.subject.toLowerCase().includes(search.toLowerCase())) return false;
  ...
  }, [tickets, search, type, channel, status, priority]);

  const columns = React.useMemo<ColumnDef<SupportListRow, any>[]>(
    () => [
      {
        id: "client",
        header: "Client",
      ...
      },
      {
        id: "updatedAt",
        header: "Mis à jour",
        cell: ({ row }) => <span className="text-sm text-stone-500 dark:text-stone-400">{formatDate(row.original.updated_at)}</span>,
      },
    ],
    []
  );
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-0.5 dark:text-stone-400">{filtered.length} résultat{filtered.length > 1 ? "s" : ""} sur {tickets.length}</p>
        </div>
      ...
      <NewTicketDialog open={formOpen} onOpenChange={setFormOpen} clients={clients} />
    </div>
  );
}
