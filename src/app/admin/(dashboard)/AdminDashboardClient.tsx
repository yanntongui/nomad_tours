...
import { createVisaRequestAction } from "./visas/actions";
import { createEventRequestAction } from "./evenementiel/actions";
import { createClientAction } from "./clients/actions";
import { addPaymentAction } from "./reservations/actions";
import type { BookingListRow, PaymentScheduleListRow } from "@/lib/server/bookings";
import type { TripSummaryRow } from "@/lib/server/trips";
...
type TimelineEntry = Tables<"booking_timeline"> & {
...
const CONTACT_LABELS: Record<Tables<"clients">["preferred_contact"], string> = {
  EMAIL: "Email",
  PHONE: "Téléphone",
  WHATSAPP: "WhatsApp",
};
const VIP_LABELS: Record<Tables<"clients">["vip_tier"], string> = {
  STANDARD: "Standard",
  SILVER: "Silver",
  GOLD: "Gold",
  PLATINUM: "Platinum",
};
const PAYMENT_METHOD_LABELS: Record<Tables<"payments">["method"], string> = {
...
const PAYMENT_STATUS_LABELS: Record<Tables<"payments">["status"], string> = {
  PENDING: "En attente",
  PARTIAL: "Partiel",
  PAID: "Payé",
  FAILED: "Échoué",
...
function VisaQuickCreateDialog({
  open,
  onOpenChange,
  clients,
  agents,
  ...
  const valid = clientId.length > 0 && country.length > 0 && feeXOF > 0 && agentId.length > 0;

  async function handleSubmit() {
    const result = await createVisaRequestAction(
      { client_id: clientId, country, fee_xof: feeXOF, agent_id: agentId },
      user.name
    );
    if (result.error || !result.data) return;
    onOpenChange(false);
    router.push(`/admin/visas/${result.data.id}`);
  }
...
function EventQuickCreateDialog({
  open,
  onOpenChange,
  clients,
  agents,
  ...
  const valid = clientId.length > 0 && type.length > 0 && guestCount > 0 && budgetXOF > 0 && eventDate.length > 0 && agentId.length > 0;

  async function handleSubmit() {
    const result = await createEventRequestAction(
      {
        client_id: clientId,
        type,
    ...
    if (result.error || !result.data) return;
    onOpenChange(false);
    router.push(`/admin/evenementiel/${result.data.id}`);
  }
...
function ClientQuickCreateDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const [preferredContact, setPreferredContact] = React.useState<Tables<"clients">["preferred_contact"]>("EMAIL");
  const [vipTier, setVipTier] = React.useState<Tables<"clients">["vip_tier"]>("STANDARD");
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select value={preferredContact} onValueChange={(v) => setPreferredContact(v as Tables<"clients">["preferred_contact"])}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {(Object.keys(CONTACT_LABELS) as Tables<"clients">["preferred_contact"][]).map((c) => (
            ...
            <div className="space-y-1.5">
              ...
              <Select value={vipTier} onValueChange={(v) => setVipTier(v as Tables<"clients">["vip_tier"])}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {(Object.keys(VIP_LABELS) as Tables<"clients">["vip_tier"][]).map((v) => (
...
function PaymentQuickCreateDialog({ open, onOpenChange, bookings }: { open: boolean; onOpenChange: (open: boolean) => void; bookings: BookingListRow[] }) {
  const router = useRouter();
  const { user } = useAdminRole();
  const [bookingId, setBookingId] = React.useState("");
  const [amountXOF, setAmountXOF] = React.useState(0);
  const [method, setMethod] = React.useState<Tables<"payments">["method"]>("MOBILE_MONEY_MTN");
  const [status, setStatus] = React.useState<Tables<"payments">["status"]>("PAID");
        ...
        <div className="space-y-3">
          <div className="space-y-1.5">
            ...
            <Select value={bookingId} onValueChange={setBookingId}>
              ...
              <SelectContent>
                {bookings.map((b) => (
                  <SelectItem key={b.id} value={b.id}>{b.booking_number} — {b.clients?.name ?? "—"}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label>Méthode</Label>
              <Select value={method} onValueChange={(v) => setMethod(v as Tables<"payments">["method"])}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {(Object.keys(PAYMENT_METHOD_LABELS) as Tables<"payments">["method"][]).map((m) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label>Statut</Label>
              <Select value={status} onValueChange={(v) => setStatus(v as Tables<"payments">["status"])}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {(Object.keys(PAYMENT_STATUS_LABELS) as Tables<"payments">["status"][]).map((s) => (
...
interface AdminDashboardClientProps {
  ...
  schedules: PaymentScheduleListRow[];
...
export function AdminDashboardClient({ bookings, schedules, trips, clients, timeline, agents }: AdminDashboardClientProps) {
  const [visaOpen, setVisaOpen] = React.useState(false);
  const [eventOpen, setEventOpen] = React.useState(false);
  const [clientOpen, setClientOpen] = React.useState(false);
  const [paymentOpen, setPaymentOpen] = React.useState(false);

  const totalRevenue = bookings.reduce((sum, b) => sum + b.paid_xof, 0);
  const totalBookings = bookings.length;
  const ongoingTrips = trips.filter((t) => t.status === "ONGOING").length;
  const latePayments = schedules.filter((s) => s.status === "LATE");
  const urgentBookings = bookings.filter((b) => b.urgent);
  ...
  return (
    <div className="space-y-6">
              ...
              {ongoingTripsList.map((trip) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-[11px] text-stone-400 dark:text-stone-500">{trip.bookings?.booking_number}</span>
                  </div>
                  <p className="text-sm font-semibold text-stone-800 dark:text-stone-100 truncate">{trip.bookings?.reference_label ?? "Voyage"}</p>
                  <p className="text-xs text-stone-500 dark:text-stone-400 mt-0.5 truncate">{trip.bookings?.clients?.name ?? "—"}</p>
                  {trip.admin_profiles?.name && <p className="text-[11px] text-stone-400 dark:text-stone-500 mt-1">Guide : {trip.admin_profiles.name}</p>}
                </Link>
              ))}
            </div>
          )}
      ...
      <VisaQuickCreateDialog open={visaOpen} onOpenChange={setVisaOpen} clients={clients} agents={agents} />
      <EventQuickCreateDialog open={eventOpen} onOpenChange={setEventOpen} clients={clients} agents={agents} />
      <ClientQuickCreateDialog open={clientOpen} onOpenChange={setClientOpen} />
      <PaymentQuickCreateDialog open={paymentOpen} onOpenChange={setPaymentOpen} bookings={bookings} />
    </div>
  );
}
