...
import { useClientsStore } from "@/lib/admin/store/clients-store";
...
export default function ClientsPage() {
  ...
  const allTags = React.useMemo(
    () => Array.from(new Set(store.clients.flatMap((c) => c.tags))).sort(),
    [store.clients]
  );
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-stone-500 mt-0.5 dark:text-stone-400">{filtered.length} résultat{filtered.length > 1 ? "s" : ""} sur {store.clients.length}</p>
      </div>
