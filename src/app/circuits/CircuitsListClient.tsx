...
import type { PublicCircuit } from "@/components/CircuitCard";

interface CircuitsListClientProps {
  circuits: PublicCircuit[];
}

export function CircuitsListClient({ circuits }: CircuitsListClientProps) {
  ...
  const filteredCircuits = useMemo(() => {
    return circuits.filter((circuit) => {
      ...
      if (maxDuration === "SHORT") matchesDuration = circuit.duration_days <= 3;
      if (maxDuration === "MEDIUM") matchesDuration = circuit.duration_days > 3 && circuit.duration_days <= 6;
      if (maxDuration === "LONG") matchesDuration = circuit.duration_days > 6;

      return matchesSearch && matchesTheme && matchesDuration;
    });
  }, [circuits, searchQuery, selectedTheme, maxDuration]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredCircuits.length > 0 ? (
          filteredCircuits.map((circuit) => (
            <CircuitCard key={circuit.id} circuit={circuit} onOpenQuoteModal={onOpenQuoteModal} />
          ))
        ) : (
